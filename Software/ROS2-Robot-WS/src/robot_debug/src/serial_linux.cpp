#include "../include/serial_linux/serial_linux.hpp"
...
Serial::Serial(const std::string &port_name_):Serial_EC(0)
{
    ...
    {
        Serial_EC = 1;
        std::cout << "Serial_port make_shared Failed" << std::endl;
    }
...
void Serial::read_from_serial()
{
    ...
    std::copy(std::istream_iterator<unsigned char>(std::istream(&response) >> std::noskipws),
...
// 串口发送函数
bool Serial::SerialWrite(short * MessageData, unsigned int MessageLenth, unsigned int MessageCode)
{
    // 计算发送消息长度
    ...
    for(unsigned int i = 0; i < MessageLenth; i++)
    {
        SendUnion.data = MessageData[i]; //更新消息Union内容
        for(int j = 0; j < 2; j++)
        {
            //送入缓冲区
...
// 串口接收函数, 成功返回true
bool Serial::SerialRead(short * MessageData, unsigned int * MessageLenth, unsigned int * MessageCode)
{
    // 接收数据缓冲区
    unsigned char ReceiveBuf[255] = {0};
    ...
    *MessageLenth = (ReceiveLength - 7)/2;
    // debug ReceiveLength
    // std::cout << "ReceiveLength: " << std::dec << (unsigned int)ReceiveLength << std::endl;

    // 消息码传递
    *MessageCode = (unsigned int)ReceiveBuf[3];
    // debug MessageCode
    // std::cout << "MessageCode: " << std::setw(2) << std::hex << *MessageCode << std::endl;
